...
namespace ExchangeDocument.PresentationLayer.Controllers
{
    ...
    public class DocumentsController : ControllerBase
    {
        ...
        {
            // Lấy UserId từ Claims - bắt buộc để kiểm tra kho cá nhân
            if (!int.TryParse(User.FindFirstValue(ClaimTypes.NameIdentifier), out int userId))
                return Unauthorized();

            var (documentDetail, error) = await _documentService.GetMyDocumentByIdAsync(id, userId);
            ...
            if (error != null)
            {
                ...
                if (error == "Forbidden")
                    return Forbid();
                if (error == "Conflict")
                    return Conflict(new { message = "Tài liệu không còn trong kho hoặc ở trạng thái không hợp lệ." });
